...
could be a case of card reader which acts as an adapter between memory card 
and a laptop. You plugin the memory card into card reader and card reader into 
the laptop so that memory card can be read via laptop.
 */
public class MobilePhoneCharager implements Target {
	...
	public Volt get10Volt() {
		...
		Volt convertedVolt = new Volt(volt.getVolt() / 2);
